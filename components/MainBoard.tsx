...
const SAMPLE_JD = `JOB TITLE: Senior Technical Recruiter / Talent Lead
LOCATION: Remote / New Jersey
SALARY: $140,000 - $175,000 + Performance Bonus

REQUIREMENTS:
- 8+ years in full-cycle recruiting for Skilled Trades or Tech.
- Experience with AI-driven sourcing tools and CRM automation.
- Ability to manage high-volume pipelines and executive stakeholders.
- Expertise in SWOT analysis of candidates and strategic interviewing.
- Strong proficiency in building divisions for the Mid-Atlantic region.`;

const SAMPLE_RESUME = `DAN SIMS
Lead Recruiter | East Windsor, NJ | d.sims@email.com

EXECUTIVE SUMMARY:
High-impact Lead Recruiter at SNI Companies with a track record of scaling specialized divisions. Expert in AI-integrated talent acquisition workflows and Skilled Trades recruitment.

PROFESSIONAL EXPERIENCE:
SNI Companies | Lead Recruiter | 2024 - Present
- Promoted to lead the new Skilled Trades Division for the Mid-Atlantic region.
- Integrated AI tools to reduce screening time by 40%.
- Manage full-cycle recruitment for enterprise accounts in NJ, PA, and MD.

SKILLS:
- AI Sourcing, CRM Optimization, SWOT Metrics, Strategic Talent Acquisition.`;
...
export default function MainBoard() {
  ...
  // Elite Status State (Synced with Clerk)
  ...
  // 2. CLERK METADATA SYNC (Ensures Elite status unlocks on return)
  useEffect(() => {
  ...
  // 3. POST-CHECKOUT RECOVERY LOGIC
  useEffect(() => {
    const handleSync = async () => {
      if (!isLoaded || !isSignedIn) return;

      // Handle the "Interrupted Upgrade" flow
      ...
      // If they just paid, we poll the server to catch the Webhook update
      if (!isPro) {
        console.log("Checking for account activation...");
        await user.reload();
        
        // Final attempt after delay to give Stripe time to talk to Clerk
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast("System failed to read document.", "error");
    }
  };
  ...
  const downloadPDF = () => {
    ...
    doc.save(`RecruitIQ_${name}_Report.pdf`);
  ...
  const handleScreen = async () => {
    ...
    if (!jdReady || !resumeReady) {
      showToast("Load all parameters before synthesis.", "error");
      return;
    }
    ...
    try {
      ...
      const prompt = `Act as an expert technical recruiter. Analyze JD: ${jdText} and Resume: ${resumeText}. Extract the candidate name, assign a match score from 0-100, provide a detailed summary, identify 3 core strengths, 3 execution gaps, 5 deep-dive interview questions, and a high-converting outreach email. 
      Return ONLY valid JSON in this format: 
      {"candidate_name": "Name", "score": 0, "summary": "...", "strengths": [], "gaps": [], "questions": [], "outreach_email": "..."}`;
      ...
      showToast("Intelligence Stream Captured!");
    } catch (err) {
      showToast("API Sync Error. Verify Credentials.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!isLoaded) return (
    <div className="min-h-screen bg-[#0B1120] flex items-center justify-center flex-col gap-4">
      ...
      <p className="text-indigo-400 font-bold uppercase tracking-[0.2em] text-[10px]">Booting IQ Engine</p>
    </div>
  );
    ...
    <div className="relative p-4 md:p-10 max-w-7xl mx-auto space-y-8 text-white bg-[#0B1120] min-h-screen pt-20">
      ...
      <div className="flex justify-between items-center mb-12 border-b border-slate-800/50 pb-8">
        <div className="flex items-center gap-5">
            ...
            <div className="hidden md:block">
                ...
                <p className="text-[10px] text-indigo-400 font-black uppercase tracking-[0.3em] mt-1">Skilled Trades Division</p>
            </div>
        </div>
        <div className="flex items-center gap-6">
            <div className={`px-5 py-2.5 rounded-full text-[10px] font-black border flex items-center gap-3 transition-all ${isPro ? 'border-emerald-500 text-emerald-400 bg-emerald-500/5' : 'border-indigo-500 text-indigo-400 bg-indigo-500/5'}`}>
                {isPro ? <Shield className="w-3 h-3 fill-current" /> : <Target className="w-3 h-3" />}
                {isPro ? "ELITE ACCESS" : `TRIAL SCANS: ${3 - scanCount} LEFT`}
            </div>
            {!isSignedIn ? (
      ...
      <div className="grid md:grid-cols-3 gap-6">
          <div onClick={() => setActiveTab('jd')} className={`group p-8 rounded-[2rem] border cursor-pointer transition-all hover:scale-[1.02] ${jdReady ? 'bg-indigo-900/10 border-indigo-500/50' : 'bg-slate-900/40 border-slate-800'}`}>
              ...
              <p className="text-xs text-slate-500 font-bold uppercase tracking-tighter">Ingest Requirements</p>
          </div>
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        ...
        <div className="bg-[#111827] p-8 md:p-10 rounded-[3rem] border border-slate-800 flex flex-col h-[850px] shadow-2xl relative">
            ...
            <textarea 
              className="flex-1 bg-[#0B1120] resize-none outline-none text-slate-300 p-8 border border-slate-800 rounded-[2rem] text-xs font-mono leading-loose focus:border-indigo-500/50 transition-all"
              value={activeTab === 'jd' ? jdText : resumeText} 
              onChange={(e) => activeTab === 'jd' ? setJdText(e.target.value) : setResumeText(e.target.value)}
              placeholder={activeTab === 'jd' ? "Awaiting Job Parameters..." : "Awaiting Candidate Blueprint..."}
            />
            ...
            >
              ...
              {loading ? "Synthesizing..." : "Engage AI Intelligence Engine"}
            </button>
        </div>
            ...
            {analysis ? (
              ...
              <div className="h-full border-4 border-dashed border-slate-800/50 rounded-[4rem] flex flex-col items-center justify-center text-slate-700 font-black text-[10px] uppercase tracking-[0.5em] gap-8 text-center p-16">
                ...
                <div>
                    ...
                    <p className="text-[8px] opacity-40 normal-case tracking-widest uppercase">Connect JD & Resume to execute analysis</p>
                </div>
              </div>
            )}
        </div>
