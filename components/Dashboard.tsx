...
export default function Dashboard() {
  ...
  return (
    <div className="relative p-6 md:p-10 max-w-7xl mx-auto space-y-8 text-white bg-[#0B1120] min-h-screen pt-20">
      ...
      {toast.show && (
        <div className="fixed top-10 left-1/2 -translate-x-1/2 z-[200] px-6 py-3 rounded-2xl bg-indigo-600 shadow-2xl border border-indigo-400">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="bg-[#111827] p-8 rounded-[2.5rem] border border-slate-800 flex flex-col h-[750px] shadow-2xl">
            ...
            <div className="flex gap-3 mb-4">
              <label className="flex-1 text-center cursor-pointer bg-slate-800/50 py-3 rounded-xl text-[10px] font-bold text-slate-400 hover:text-white border border-slate-700">Upload Docx<input type="file" accept=".docx" onChange={handleFileUpload} className="hidden" /></label>
              <button onClick={() => {setJdText(SAMPLE_JD); setResumeText(SAMPLE_RESUME); showToast("Samples loaded!");}} className="flex-1 bg-slate-800/50 py-3 rounded-xl text-[10px] font-bold text-slate-400 border border-slate-700 hover:text-white">Load Samples</button>
            </div>
            <textarea className="flex-1 bg-[#0B1120] resize-none outline-none text-slate-300 p-6 border border-slate-800 rounded-2xl text-xs font-mono focus:border-indigo-500 transition-colors" value={activeTab === 'jd' ? jdText : resumeText} onChange={(e) => activeTab === 'jd' ? setJdText(e.target.value) : setResumeText(e.target.value)} placeholder="Paste content here..." />
            <button onClick={handleScreen} disabled={loading} className="mt-6 py-5 rounded-2xl font-black uppercase text-xs bg-indigo-600 shadow-2xl flex items-center justify-center gap-3 hover:bg-indigo-500 transition-all">{loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Zap className="w-5 h-5 fill-white" />}Execute Elite AI Screen →</button>
        </div>
            ...
            {analysis ? (
              <div className="space-y-6 animate-in fade-in zoom-in-95">
                <div className="bg-[#111827] border border-slate-800 p-8 rounded-[2.5rem] text-center shadow-2xl relative overflow-hidden group">
                  ...
                  <button onClick={downloadPDF} className="bg-slate-800 hover:bg-slate-700 text-indigo-400 px-6 py-3 rounded-xl text-[10px] font-bold uppercase border border-slate-700 flex items-center gap-2 mx-auto"><Download className="w-3 h-3" /> Download Report</button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-emerald-500/5 border border-emerald-500/20 p-6 rounded-3xl text-[11px]">
                    ...
                    <div className="space-y-3">{analysis.strengths.map((s: string, i: number) => <p key={i} className="text-slate-200 leading-relaxed">• {s}</p>)}</div>
                  </div>
                  <div className="bg-rose-500/5 border border-rose-500/20 p-6 rounded-3xl text-[11px]">
                    ...
                    <div className="space-y-3">{analysis.gaps.map((g: string, i: number) => <p key={i} className="text-slate-200 leading-relaxed">• {g}</p>)}</div>
                  </div>
                </div>
                ...
                <div className="bg-[#111827] border border-slate-800 p-8 rounded-3xl text-center">
                    ...
                    <div className="bg-slate-900/50 rounded-2xl p-6 mb-6 text-left border border-slate-800"><p className="text-[11px] text-slate-300 whitespace-pre-wrap leading-relaxed">{analysis.outreach_email}</p></div>
              ...
              <div className="h-full border-2 border-dashed border-slate-800 rounded-[3rem] flex flex-col items-center justify-center text-slate-600 font-black text-[10px] uppercase gap-6 text-center p-12"><Zap className="w-8 h-8 opacity-20" /><p>Waiting for Candidate Data...</p></div>
            )}
        </div>
      </div>

      <footer className="mt-12 border-t border-slate-800 pt-8 pb-12 text-center text-[10px] uppercase font-bold text-slate-500">
        <button onClick={() => setShowSupportModal(true)} className="hover:text-indigo-400 mb-4 transition-colors">Support</button>
        <p>&copy; {new Date().getFullYear()} Recruit-IQ.</p>
      </footer>
          ...
          <div className="relative w-full max-w-4xl animate-in zoom-in-95 duration-500">
            ...
            <div className="relative bg-[#0F172A] border border-slate-700/50 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row">
              <div className="p-10 md:w-3/5 flex flex-col justify-center">
                 <h2 className="text-5xl font-black text-white mb-4 leading-[1.1] tracking-tighter">Hire Your Next Star <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400">In Seconds.</span></h2>
                 <p className="text-slate-400 text-sm mb-8 leading-relaxed font-medium">Unlock unlimited resume screening, deep AI personality analysis, and outreach tools instantly.</p>
                 <a href={finalStripeUrl} className="block w-full py-5 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-center text-white font-black rounded-2xl uppercase text-xs shadow-2xl hover:scale-[1.02] transition-all"><span className="flex items-center justify-center gap-2"><Sparkles className="w-4 h-4 fill-white" /> Start 3-Day Free Trial</span></a>
                 <button onClick={() => setShowLimitModal(false)} className="text-center text-[10px] text-slate-500 mt-6 uppercase font-bold w-full hover:text-white transition-colors">Continue Limited Access</button>
              </div>
              <div className="md:w-2/5 bg-[#111827]/80 backdrop-blur-md border-l border-slate-800 p-12 flex flex-col justify-center relative"><div className="space-y-8"><div className="flex items-start gap-4"><Zap className="w-5 h-5 text-indigo-400" /><div><h4 className="text-white font-black uppercase text-[10px] tracking-widest">Instant Analysis</h4></div></div><div className="flex items-start gap-4"><Shield className="w-5 h-5 text-purple-400" /><div><h4 className="text-white font-black uppercase text-[10px] tracking-widest">Deep Gaps Check</h4></div></div><div className="flex items-start gap-4"><Mail className="w-5 h-5 text-blue-400" /><div><h4 className="text-white font-black uppercase text-[10px] tracking-widest">Smart Outreach</h4></div></div></div></div>
            </div>
          </div>
        </div>
      )}
      ...
      {showSupportModal && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 backdrop-blur-xl bg-slate-950/90">
          <div className="bg-[#0F172A] border border-slate-700 p-10 rounded-[2.5rem] max-w-lg w-full shadow-2xl text-center">
            <h2 className="text-2xl font-black mb-6 uppercase text-white">Support</h2>
            <textarea required className="w-full h-40 bg-[#0B1120] border border-slate-800 rounded-2xl p-6 text-[12px] text-white outline-none resize-none mb-6 focus:border-indigo-500" placeholder="How can we help?" value={supportMessage} onChange={(e) => setSupportMessage(e.target.value)} />
