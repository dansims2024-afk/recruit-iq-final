...
export default function Dashboard() {
  ...
  // --- THE INSTANT UNLOCK & STRIPE REDIRECT LOGIC ---
  useEffect(() => {
    const handleReturnFlow = async () => {
      if (!isLoaded || !isSignedIn) return;

      // Check if we need to force redirect to Stripe (triggered from Log In button)
      ...
      // If returning but not Pro, force a reload to catch metadata updates
      if (!isPro) {
        await user?.reload();
  ...
  return (
    <div className="relative p-4 md:p-10 max-w-7xl mx-auto space-y-8 text-white bg-[#0B1120] min-h-screen pt-20">
      ...
      {/* RESTORED HEADER WITH LOG IN BUTTON */}
            ...
            {analysis ? (
              <div className="space-y-6 animate-in fade-in zoom-in-95">
                <div className="bg-[#111827] border border-slate-800 p-8 rounded-[2.5rem] text-center shadow-2xl relative">
                  ...
                  <button onClick={downloadPDF} className="bg-slate-800 hover:bg-slate-700 text-indigo-400 px-6 py-3 rounded-xl text-[10px] font-bold uppercase border border-slate-700 flex items-center gap-2 mx-auto">
                ...
                <div className="bg-[#111827] border border-slate-800 p-8 rounded-3xl text-center">
                    ...
                    <button onClick={() => {navigator.clipboard.writeText(analysis.outreach_email); showToast("Copied!", "success");}} className="w-full py-4 bg-slate-800 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2"><Copy className="w-3 h-3" /> Copy Email</button>
                </div>
              </div>
            ) : (
      ...
      {/* --- ELITE SALES MODAL (Premium Version) --- */}
      {showLimitModal && (
                 ...
                 <div className="relative">
                    {!isSignedIn ? (
                        <SignUpButton mode="modal" forceRedirectUrl={STRIPE_URL}>
                            <button onClick={() => sessionStorage.setItem('trigger_stripe', 'true')} className="group block w-full py-5 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-center text-white font-black rounded-2xl uppercase tracking-wider text-xs shadow-2xl">
                        ...
                        <a href={finalStripeUrl} className="group block w-full py-5 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-center text-white font-black rounded-2xl uppercase tracking-wider text-xs shadow-2xl">
                             <span className="flex items-center justify-center gap-2">
                                <Sparkles className="w-4 h-4 fill-white" /> Start 3-Day Free Trial
                            </span>
                        </a>
