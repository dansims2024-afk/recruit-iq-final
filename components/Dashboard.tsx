  ...
  const handleScreen = async () => {
    ...
    try {
      const response = await fetch('/api/generate', { 
        method: "POST", 
        body: JSON.stringify({ prompt: `JD: ${jdText} \n RESUME: ${resumeText}` }) 
      });
    ...
    <div className="relative p-4 md:p-10 max-w-7xl mx-auto space-y-12 text-white bg-[#0B1120] min-h-screen pt-20 font-sans tracking-tight">
      ...
      <div className="grid md:grid-cols-3 gap-8">
          <div onClick={() => setActiveTab('jd')} className={`p-10 rounded-[2.5rem] border cursor-pointer transition-all hover:scale-[1.02] ${jdReady ? 'bg-indigo-900/20 border-emerald-500' : 'bg-slate-800/30 border-slate-700'}`}>
              ...
              <h4 className="uppercase text-xs font-black tracking-wide mb-3">Define JD</h4>
              <p className="text-sm text-slate-400 leading-relaxed font-medium">Set the benchmark for AI screening.</p>
          </div>
          <div onClick={() => setActiveTab('resume')} className={`p-10 rounded-[2.5rem] border cursor-pointer transition-all hover:scale-[1.02] ${resumeReady ? 'bg-indigo-900/20 border-emerald-500' : 'bg-slate-800/30 border-slate-700'}`}>
              ...
              <p className="text-sm text-slate-400 leading-relaxed font-medium">Upload a PDF or Word resume.</p>
          </div>
          <div className={`p-10 rounded-[2.5rem] border transition-all ${analysis ? 'bg-indigo-900/20 border-indigo-500' : 'bg-slate-800/30 border-slate-700'}`}>
              ...
              <h4 className="uppercase text-xs font-black tracking-wide mb-3">Intelligence</h4>
              <p className="text-sm text-slate-400 leading-relaxed font-medium">Generate Match Scores & Insights.</p>
          </div>
      </div>
              ...
              <div className="space-y-10 animate-in fade-in zoom-in-95 duration-500">
                ...
                <div className="bg-[#111827] border border-slate-800 p-12 rounded-[3rem] text-center">
                    ...
                    <div className="bg-slate-950/80 rounded-[2rem] p-10 mb-10 text-left border border-slate-800 shadow-2xl">
                        <p className="text-sm text-slate-300 whitespace-pre-wrap leading-relaxed font-sans font-bold">{analysis.outreach_email}</p>
                    </div>
                    <button onClick={() => {navigator.clipboard.writeText(analysis.outreach_email); showToast("Email Template Copied!");}} className="w-full py-6 bg-slate-800 hover:bg-slate-700 rounded-2xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-4 border border-slate-700 shadow-xl">
      ...
      {showLimitModal && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 backdrop-blur-3xl bg-slate-950/90 animate-in fade-in duration-300">
          <div className="bg-[#0F172A] border border-slate-700/50 rounded-[3.5rem] p-16 max-w-4xl w-full shadow-2xl flex flex-col md:flex-row overflow-hidden group">
              <div className="relative p-4 md:w-3/5">
                 ...
                 {!isSignedIn ? (
                    <SignUpButton mode="modal">
                        <button onClick={() => sessionStorage.setItem('trigger_stripe', 'true')} className="block w-full py-8 bg-indigo-600 text-center text-white font-black rounded-[2.5rem] uppercase tracking-widest transition-all text-xs shadow-2xl shadow-blue-500/40">Start Free Trial</button>
                    </SignUpButton>
                 ) : (
                    <div className="space-y-6">
                        ...
                        <button onClick={handleVerifySubscription} disabled={verifying} className="block w-full py-4 bg-slate-800 text-slate-400 font-black rounded-2xl uppercase tracking-widest transition-all text-xs border border-slate-700 shadow-xl">{verifying ? "VERIFYING..." : "SYNC ACCOUNT"}</button>
                    </div>
                 )}
              ...
              <div className="hidden md:flex md:w-2/5 bg-slate-900/50 border-l border-slate-800/50 flex-col items-center justify-center p-20 text-center shadow-inner">
                 ...
                 <h3 className="font-black text-white text-4xl uppercase tracking-tighter mb-8 text-center">Elite Status</h3>
      ...
      {showSupportModal && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 backdrop-blur-xl bg-slate-950/90">
          <div className="bg-[#0F172A] border border-slate-700 p-12 rounded-[3rem] max-w-lg w-full shadow-2xl text-center">
            ...
            <textarea required className="w-full h-48 bg-[#0B1120] border border-slate-800 rounded-3xl p-8 text-sm text-white outline-none resize-none mb-10 focus:border-indigo-500 transition-colors" placeholder="How can we help?" value={supportMessage} onChange={(e) => setSupportMessage(e.target.value)} />
            <div className="flex gap-6"><button onClick={() => {showToast("Message sent!"); setShowSupportModal(false);}} className="flex-1 py-5 bg-indigo-600 hover:bg-indigo-500 rounded-2xl font-black uppercase text-xs tracking-widest transition-all shadow-xl">Send</button><button onClick={() => setShowSupportModal(false)} className="px-10 py-5 bg-slate-800 hover:bg-slate-700 rounded-2xl font-black uppercase text-xs tracking-widest transition-all">Cancel</button></div>
          </div>
        </div>
      )}
    </div>
