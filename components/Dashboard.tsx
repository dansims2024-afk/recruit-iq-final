...
// --- FIXED: YOUR REAL STRIPE LINK IS NOW HARDCODED HERE ---
...
export default function Dashboard() {
  ...
  // FAIL-SAFE REDIRECT: Uses local path to bypass DNS subdomain issues
  ...
  // This ensures that if you are logged in but haven't paid, you MUST see the checkout button.
  if (isSignedIn && !isPro) {
    return (
                ...
                <div className="mt-6">
                  ...
                  <SignInButton>
                     <button className="text-slate-500 text-[10px] font-bold uppercase tracking-widest hover:text-white transition-colors flex items-center justify-center gap-2 mx-auto">
                        <LogOut className="w-3 h-3" /> Sign Out (Reset)
                     </button>
                  </SignInButton>
                </div>
            </div>
