...
export default function Dashboard() {
  ...
  const downloadPDF = () => {
    ...
    const maxLength = Math.max(strengths.length, gaps.length);
  ...
  const handleScreen = async () => {
    ...
    try {
      ...
      console.error(err);
      showToast("AI Engine Error. Check your API key or data format.", "error");
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-emerald-500/5 border border-emerald-500/20 p-6 rounded-3xl text-[11px]">
                    ...
                    <div className="space-y-3">
                        {analysis.strengths && analysis.strengths.map((s: string, i: number) => <p key={i} className="text-slate-200 leading-relaxed">• {s}</p>)}
                    </div>
                  </div>
                  <div className="bg-rose-500/5 border border-rose-500/20 p-6 rounded-3xl text-[11px]">
                    ...
                    <div className="space-y-3">
                        {analysis.gaps && analysis.gaps.map((g: string, i: number) => <p key={i} className="text-slate-200 leading-relaxed">• {g}</p>)}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#111827] border border-slate-800 p-8 rounded-3xl">
                  ...
                  <div className="space-y-4">
                    {analysis.questions && analysis.questions.map((q: string, i: number) => (
