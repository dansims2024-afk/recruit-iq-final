...
export default function Dashboard() {
  ...
  const handleVerifySubscription = async () => {
    ...
    try {
      ...
      } else { showToast("Payment not found yet.", "error"); }
  ...
  const handleScreen = async () => {
    ...
    if (!jdReady || !resumeReady) {
      showToast("Job Description and Resume are required.", "error");
      return;
    }
    setLoading(true);
    try {
      const prompt = `Analyze JD: ${jdText} and Resume: ${resumeText}. Extract candidate name, score 0-100, summary, 3 strengths, 3 gaps, 5 questions, and outreach email. Return JSON: {"candidate_name": "Name", "score": 0, "summary": "...", "strengths": [], "gaps": [], "questions": [], "outreach_email": "..."}`;
        ...
        <div className="flex items-center gap-4">
            ...
            {!isSignedIn ? (
                <SignInButton mode="modal">
                    <button onClick={() => sessionStorage.setItem('trigger_stripe', 'true')} className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all shadow-lg shadow-indigo-500/20">Log In</button>
                </SignInButton>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="bg-[#111827] p-8 rounded-[2.5rem] border border-slate-800 flex flex-col h-[750px] shadow-2xl relative">
            ...
            <div className="flex gap-3 mb-4">
              <label className="flex-1 text-center cursor-pointer bg-slate-800/50 py-3 rounded-xl text-[10px] font-bold uppercase text-slate-400 hover:text-white border border-slate-700 transition-all">Upload Docx<input type="file" accept=".docx" onChange={handleFileUpload} className="hidden" /></label>
            ...
            <textarea className="flex-1 bg-[#0B1120] resize-none outline-none text-slate-300 p-6 border border-slate-800 rounded-2xl text-xs font-mono focus:border-indigo-500 transition-colors custom-scrollbar" value={activeTab === 'jd' ? jdText : resumeText} onChange={(e) => activeTab === 'jd' ? setJdText(e.target.value) : setResumeText(e.target.value)} placeholder={activeTab === 'jd' ? "Paste JD..." : "Paste Resume..."}/>
            <button onClick={handleScreen} disabled={loading} className="mt-6 py-5 rounded-2xl font-black uppercase text-xs bg-indigo-600 shadow-2xl flex items-center justify-center gap-3 hover:bg-indigo-500 transition-all disabled:opacity-50 group">
            ...
            {analysis ? (
              <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
                <div className="bg-[#111827] border border-slate-800 p-8 rounded-[2.5rem] text-center shadow-2xl relative overflow-hidden group">
                  ...
                  <button onClick={downloadPDF} className="bg-slate-800 hover:bg-slate-700 text-indigo-400 px-6 py-3 rounded-xl text-[10px] font-bold uppercase border border-slate-700 transition-all flex items-center gap-2 mx-auto"><Download className="w-3 h-3" /> Download Elite Report</button>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-emerald-500/5 border border-emerald-500/20 p-6 rounded-3xl text-[11px]"><h4 className="text-emerald-400 font-bold uppercase mb-4 text-[9px] tracking-widest flex items-center gap-2"><Check className="w-3 h-3" /> Key Strengths</h4><div className="space-y-3">{analysis.strengths.map((s: any, i: any) => <p key={i} className="text-slate-200 leading-relaxed">• {s}</p>)}</div></div>
                  <div className="bg-rose-500/5 border border-rose-500/20 p-6 rounded-3xl text-[11px]"><h4 className="text-rose-400 font-bold uppercase mb-4 text-[9px] tracking-widest flex items-center gap-2"><Shield className="w-3 h-3" /> Critical Gaps</h4><div className="space-y-3">{analysis.gaps.map((g: any, i: any) => <p key={i} className="text-slate-200 leading-relaxed">• {g}</p>)}</div></div>
                </div>
                <div className="bg-[#111827] border border-slate-800 p-8 rounded-3xl"><h4 className="text-indigo-400 font-bold uppercase text-[9px] mb-6 tracking-widest flex items-center gap-2"><HelpCircle className="w-3 h-3" /> Interview Guide</h4><div className="space-y-4">{analysis.questions.map((q: any, i: any) => (<div key={i} className="p-4 bg-slate-800/40 rounded-2xl border border-slate-700/50 text-[11px] leading-relaxed text-slate-300">"{q}"</div>))}</div></div>
                <div className="bg-[#111827] border border-slate-800 p-8 rounded-3xl text-center"><h4 className="text-blue-400 font-bold uppercase text-[9px] mb-4 tracking-widest">Outreach Email</h4><div className="bg-slate-900/50 rounded-2xl p-6 mb-6 text-left border border-slate-800"><p className="text-[11px] text-slate-300 whitespace-pre-wrap leading-relaxed">{analysis.outreach_email}</p></div><button onClick={() => {navigator.clipboard.writeText(analysis.outreach_email); showToast("Email copied!");}} className="w-full py-4 bg-slate-800 hover:bg-slate-700 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors flex items-center justify-center gap-2"><Copy className="w-3 h-3" /> Copy Outreach Email</button></div>
              </div>
            ) : (
              <div className="h-full border-2 border-dashed border-slate-800 rounded-[3rem] flex flex-col items-center justify-center text-slate-600 font-black text-[10px] uppercase tracking-widest gap-6 text-center p-12"><div className="w-20 h-20 rounded-full bg-slate-800/30 flex items-center justify-center animate-pulse"><Zap className="w-8 h-8 opacity-20" /></div><div><p className="mb-1">Waiting for Intelligence</p></div></div>
            )}
        </div>
      </div>
          ...
          <div className="relative w-full max-w-4xl animate-in zoom-in-95 duration-500">
            ...
            <div className="relative bg-[#0F172A] border border-slate-700/50 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row">
              <div className="p-10 md:w-3/5 flex flex-col justify-center">
                 ...
                 <p className="text-slate-400 text-sm mb-8 leading-relaxed font-medium">Unlock unlimited scans, deep AI analysis, and interview guides.</p>
                 {!isSignedIn ? (
                    <SignUpButton mode="modal" afterSignUpUrl="/"><button onClick={() => sessionStorage.setItem('trigger_stripe', 'true')} className="block w-full py-5 bg-gradient-to-r from-blue-600 to-indigo-600 text-center text-white font-black rounded-2xl uppercase tracking-wider hover:scale-[1.02] transition-all text-xs shadow-2xl shadow-blue-500/40">Start 3-Day Free Trial</button></SignUpButton>
                 ) : (
                    <div className="space-y-3"><a href={finalStripeUrl} className="block w-full py-5 bg-gradient-to-r from-blue-600 to-indigo-600 text-center text-white font-black rounded-2xl uppercase tracking-wider hover:scale-[1.02] transition-all text-xs shadow-2xl shadow-blue-500/40">Start 3-Day Free Trial</a><button onClick={handleVerifySubscription} disabled={verifying} className="block w-full py-3 bg-slate-800 text-center text-slate-400 font-bold rounded-2xl uppercase tracking-widest hover:text-white transition-all text-[10px]">{verifying ? "Syncing..." : "I've Paid (Sync Account)"}</button></div>
                 )}
              ...
              <div className="hidden md:flex md:w-2/5 bg-slate-900/50 border-l border-slate-800 flex flex-col items-center justify-center p-12 text-center"><div className="w-24 h-24 bg-indigo-500/10 rounded-full flex items-center justify-center mb-8 border border-indigo-500/30 shadow-[0_0_50px_rgba(79,70,229,0.3)] animate-pulse"><Zap className="w-10 h-10 text-indigo-400 fill-indigo-400" /></div><h3 className="font-black text-white text-2xl uppercase tracking-tighter mb-4">Elite Access</h3><ul className="text-xs text-slate-400 space-y-3 font-medium"><li>✓ Unlimited Resume Scans</li><li>✓ Advanced Match Metrics</li><li>✓ Interview Guides</li><li>✓ Email Outreach</li></ul></div>
            </div>
          </div>
        </div>
      )}
      {/* Footer & Support Modals remain same ... */}
    </div>
  );
}
