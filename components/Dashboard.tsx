...
import { 
  Loader2, Copy, Check, FileText, Download, Send, 
...
ABOUT THE COMPANY:
Vertex Financial Systems is a global leader in high-frequency trading technology.
...
EXECUTIVE SUMMARY:
Strategic Technical Leader with 14 years of experience building mission-critical financial infrastructure. Expert in AWS cloud-native transformations.
...
export default function Dashboard() {
  ...
  const downloadPDF = () => {
    ...
    let yStart = 115;
    doc.setTextColor(16, 185, 129); doc.setFontSize(12); doc.text("STRENGTHS", 15, yStart);
    doc.setTextColor(244, 63, 94); doc.text("GAPS", 110, yStart);
    doc.save(`RecruitIQ_${cName}.pdf`);
  };
  ...
  if (!isLoaded) return (
    <div className="min-h-screen bg-[#0B1120] flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-slate-500 font-black uppercase tracking-widest text-[10px]">Initializing Intelligence...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative p-4 md:p-10 max-w-7xl mx-auto space-y-12 text-white bg-[#0B1120] min-h-screen pt-20 font-sans tracking-tight">
        ...
        <div className="flex items-center gap-4">
            ...
            {!isSignedIn ? (
              <SignInButton mode="modal">
                <button className="bg-indigo-600 hover:bg-indigo-500 px-6 py-2 rounded-xl text-xs font-bold uppercase">Log In</button>
              </SignInButton>
      ...
      <div className="grid md:grid-cols-3 gap-8">
          <div onClick={() => setActiveTab('jd')} className={`p-10 rounded-[2.5rem] border cursor-pointer transition-all ${jdReady ? 'bg-indigo-900/20 border-emerald-500' : 'bg-slate-800/30 border-slate-700'}`}>
              ...
              <h4 className="uppercase text-xs font-black tracking-wide mb-3">Job Description</h4>
              <p className="text-sm text-slate-400 font-medium leading-relaxed">Paste JD text or upload PDF/Word to set benchmark.</p>
          </div>
          <div onClick={() => setActiveTab('resume')} className={`p-10 rounded-[2.5rem] border cursor-pointer transition-all ${resumeReady ? 'bg-indigo-900/20 border-emerald-500' : 'bg-slate-800/30 border-slate-700'}`}>
              <div className="flex justify-between items-center mb-6">
              ...
              <h4 className="uppercase text-xs font-black tracking-wide mb-3">Candidate Resume</h4>
              <p className="text-sm text-slate-400 font-medium leading-relaxed">Upload candidate data to begin intelligence screen.</p>
          </div>
          <div className={`p-10 rounded-[2.5rem] border transition-all ${analysis ? 'bg-indigo-900/20 border-indigo-500' : 'bg-slate-800/30 border-slate-700'}`}>
              ...
              <h4 className="uppercase text-xs font-black tracking-wide mb-3">AI Intelligence</h4>
              <p className="text-sm text-slate-400 font-medium leading-relaxed">Generate match scores and strategic interview questions.</p>
          </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        <div className="bg-[#111827] p-10 rounded-[3rem] border border-slate-800 flex flex-col h-[800px] shadow-2xl relative">
            <div className="flex gap-4 mb-8">
                <button onClick={() => setActiveTab('jd')} className={`flex-1 py-5 rounded-2xl text-xs font-black uppercase flex items-center justify-center gap-3 border transition-all ${activeTab === 'jd' ? 'bg-indigo-600 border-indigo-500 text-white shadow-xl' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
                  1. Job Description
                </button>
                <button onClick={() => setActiveTab('resume')} className={`flex-1 py-5 rounded-2xl text-xs font-black uppercase flex items-center justify-center gap-3 border transition-all ${activeTab === 'resume' ? 'bg-indigo-600 border-indigo-500 text-white shadow-xl' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
                  2. Resume
                </button>
            </div>
            ...
            <div className="flex gap-4 mb-6">
              ...
              <button onClick={() => {setJdText(SAMPLE_JD); setResumeText(SAMPLE_RESUME); showToast("Elite Samples Loaded!");}} className="flex-1 bg-slate-800/50 py-4 rounded-xl text-xs font-black uppercase text-slate-400 border border-slate-700 hover:text-white transition-all">Load Samples</button>
            </div>
            ...
            {analysis ? (
              <div className="space-y-10 animate-in fade-in zoom-in-95 duration-500">
                ...
                <div className="bg-[#111827] border border-slate-800 p-12 rounded-[3rem]">
                  ...
                  <div className="space-y-6">{analysis.questions.map((q: string, i: number) => <div key={i} className="p-8 bg-slate-900/50 rounded-3xl border border-slate-800 text-sm leading-relaxed text-slate-200 font-bold italic">"{q}"</div>)}</div>
                </div>
                ...
                <div className="bg-[#111827] border border-slate-800 p-12 rounded-[3rem] text-center">
                    ...
                    <button onClick={() => {navigator.clipboard.writeText(analysis.outreach_email); showToast("Copied!");}} className="w-full py-6 bg-slate-800 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-4 border border-slate-700"><Copy className="w-5 h-5" /> Copy Email Template</button>
                </div>
              </div>
            ) : (
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 backdrop-blur-3xl bg-slate-950/90">
          <div className="relative bg-[#0F172A] border border-slate-700/50 rounded-[3.5rem] p-16 max-w-4xl w-full shadow-2xl flex flex-col md:flex-row overflow-hidden">
              <div className="relative p-4 md:w-3/5">
                 ...
                 <p className="text-slate-400 mb-12 font-black uppercase text-xs tracking-widest leading-loose">Unlimited scans and strategic interview engineering.</p>
                 <a href={finalStripeUrl} className="block w-full py-8 bg-gradient-to-r from-blue-600 to-indigo-600 text-center text-white font-black rounded-[2.5rem] uppercase tracking-widest text-xs">Unlock Elite Access</a>
                 <button onClick={() => setShowLimitModal(false)} className="mt-10 text-xs text-slate-600 font-black uppercase tracking-widest hover:text-white underline w-full">No thanks, I'll screen manually</button>
              </div>
