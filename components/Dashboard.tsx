        ...
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 backdrop-blur-3xl bg-slate-950/90">
            {!isSignedIn ? (
                <SignUpButton mode="modal" forceRedirectUrl="/?signup=true">
                    <button onClick={() => sessionStorage.setItem('pending_stripe', 'true')} className="bg-indigo-600 px-10 py-5 rounded-xl font-bold uppercase">Sign Up to Start Trial</button>
                </SignUpButton>
            ) : <a href={getStripeUrl()} className="bg-indigo-600 px-10 py-5 rounded-xl font-bold uppercase">Start Trial</a>}
        </div>
      )}
    </div>
  );
