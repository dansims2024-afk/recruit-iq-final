...
export default function Dashboard() {
  ...
  // FINAL STABLE REDIRECT: Bypass DNS subdomain issues completely
  const handleStartTrial = () => {
    sessionStorage.setItem('pending_stripe', 'true');
    // Using internal Next.js path guaranteed by Clerk locally
        ...
        <div className="flex items-center gap-4">
            ...
            {!isSignedIn ? (
                <SignInButton mode="modal">
                    <button className="bg-slate-800 hover:bg-slate-700 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-slate-700">Sign In</button>
                </SignInButton>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="bg-[#111827] p-8 rounded-[2.5rem] border border-slate-800 flex flex-col h-[750px] shadow-2xl relative">
            <div className="flex gap-3 mb-6">
                <button onClick={() => setActiveTab('jd')} className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase border ${activeTab === 'jd' ? 'bg-indigo-600 border-indigo-500' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>1. JD</button>
            ...
            <div className="flex gap-3 mb-4">
              <label className="flex-1 text-center cursor-pointer bg-slate-800/50 py-3 rounded-xl text-[10px] font-bold uppercase border border-slate-700 transition-colors">
                <Upload className="w-3 h-3 inline mr-2" /> Upload
                <input type="file" accept=".docx" onChange={handleFileUpload} className="hidden" />
              </label>
              <button onClick={() => {setJdText(SAMPLE_JD); setResumeText(SAMPLE_RESUME); showToast("Samples Loaded");}} className="flex-1 bg-slate-800/50 py-3 rounded-xl text-[10px] font-bold uppercase border border-slate-700 hover:text-white transition-all">Load Elite Samples</button>
            </div>
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/90 backdrop-blur-3xl animate-in fade-in">
          <div className="relative w-full max-w-4xl bg-[#0F172A] border-2 border-slate-700 rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row text-left">
              <div className="p-12 md:w-3/5 flex flex-col justify-center">
                 ...
                 <button onClick={() => setShowLimitModal(false)} className="text-[10px] text-slate-500 hover:text-white uppercase font-black tracking-[0.2em] mt-10 transition-colors uppercase">Dismiss</button>
              </div>
