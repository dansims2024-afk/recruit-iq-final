    ...
    const content = `
      ...
      <head>
        ...
        <style>
          ...
          .summary { font-style: italic; color: #64748b; margin-top: 10px; }
          ...
          .q-hint { font-style: italic; color: #64748b; font-size: 10pt; }
      ...
      <body>
        <div class="header">
          <div class="logo">RECRUIT-IQ</div>
          <div class="meta">Strategic Interview Intelligence | Date: ${reportDate}</div>
        </div>

        <div class="score-box">
          <span style="font-size: 12pt; text-transform: uppercase; letter-spacing: 1px; color: #64748b;">Candidate Fit Score</span>
          <span class="score-val">${analysis.score}</span>
          <div class="summary">"${analysis.summary}"</div>
        </div>

        <h1>1. Key Strengths to Validate</h1>
        <ul>${analysis.strengths.map(s => `<li>${s}</li>`).join('')}</ul>

        <h1>2. Critical Gaps to Probe</h1>
        <ul>${analysis.gaps.map(g => `<li>${g}</li>`).join('')}</ul>

        <h1>3. Deep-Dive Interview Questions</h1>
        <p><em>Use these questions to verify experience and assess cultural fit.</em></p>
        
        ${analysis.questions.map(q => `
          <div class="question-box">
            <span class="q-text">Q: ${q}</span>
            <span class="q-hint">Target Response: Look for specific metrics ("I improved X by Y%"), clear ownership of the problem, and strategic rationale.</span>
          </div>
        `).join('')}

        <div class="footer">
          Generated by Recruit-IQ AI | Confidential Candidate Analysis
        </div>
      </body>
      </html>
    `;
