...
export default function Dashboard() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (file.name.endsWith('.docx')) {
        ...
        text = extracted ? extracted : `[ERROR]. Copy/paste PDF text.`;
      } else {
  ...
  const downloadReport = () => {
    ...
    const content = `
      ...
      <body>
        <h1 style="color:#10b981">Recruit-IQ Intelligence Report</h1>
        <h2>Match Score: ${analysis.score}%</h2>
        <p>${analysis.summary}</p>
        <hr/>
        <h3>Strengths</h3><ul>${analysis.strengths.map(s => `<li>${s}</li>`).join('')}</ul>
        <h3>Gaps</h3><ul>${analysis.gaps.map(g => `<li>${g}</li>`).join('')}</ul>
        <h3>Interview Questions</h3><ul>${analysis.questions.map(q => `<li>${q}</li>`).join('')}</ul>
      </body></html>
    `;
