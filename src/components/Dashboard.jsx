...
ABOUT THE ROLE:
We are seeking a Senior Architect to lead the modernization of our trading platform. Must scale microservices for $500M daily volume.`;

const SAMPLE_RESUME = `ALEXANDER MERCER
Senior Principal Engineer | alex.mercer@example.com

PROFESSIONAL SUMMARY:
12 years experience building scalable financial systems. Led infrastructure overhaul at Innovate Financial, reducing latency by 45%.`;
...
export default function Dashboard() {
  ...
  // PROGRESS LOGIC for Green Checks
  ...
  const handleFileUpload = (e) => {
    ...
    if (file.name.endsWith('.pdf')) {
      alert("Recruit-IQ: PDF parsing is currently unavailable. Please use .docx or .txt.");
      return;
    }
  ...
  const handleScreen = async () => {
    ...
    try {
      ...
      const prompt = `Provide a match score and 2-sentence summary for JD: ${jdText} and Resume: ${resumeText}`;
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8 text-white font-sans">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="bg-[#0f172a] p-6 rounded-[2.5rem] border border-slate-800 flex flex-col h-[700px]">
            <div className="flex gap-2 mb-4 bg-black/20 p-1 rounded-2xl">
              <button onClick={() => setActiveTab('jd')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase ${activeTab === 'jd' ? 'bg-blue-600' : 'text-slate-500'}`}>Job Description</button>
              <button onClick={() => setActiveTab('resume')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase ${activeTab === 'resume' ? 'bg-indigo-600' : 'text-slate-500'}`}>Resume</button>
            </div>
            <div className="mb-4 flex gap-2">
              <label className="flex-1 text-center cursor-pointer bg-slate-800 py-3 rounded-xl text-[10px] font-black uppercase border border-slate-700">Upload File<input type="file" onChange={handleFileUpload} className="hidden" /></label>
