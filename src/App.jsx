...
export default function App() {
  ...
  // 2. Function to load the sample data manually
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col font-sans">
      <header className="bg-slate-900/80 backdrop-blur-md border-b border-white/5 h-20 flex items-center justify-between px-10 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          <button onClick={loadSampleData} className="text-[10px] text-slate-400 hover:text-blue-400 uppercase font-bold tracking-widest border border-slate-800 px-3 py-1 rounded-lg transition-all">
            Load Sample
          </button>
          <div className="flex bg-slate-950/50 border border-white/5 rounded-2xl p-1 shadow-inner">
            {['standard', 'professional', 'executive'].map(t => (
              <button key={t} onClick={() => setSimTier(t)} className={`px-6 py-2 rounded-xl text-[10px] uppercase tracking-widest font-bold transition-all ${simTier === t ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>{t}</button>
            ))}
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto w-full p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section className="bg-slate-900 rounded-3xl border border-slate-800 flex flex-col h-[600px] overflow-hidden shadow-2xl">
          <div className="flex bg-slate-950/50 border-b border-slate-800 p-2 gap-2">
            <button onClick={() => setActiveTab('jd')} className={`flex-1 py-3 text-xs uppercase font-bold rounded-xl transition-all ${activeTab === 'jd' ? 'bg-slate-800 text-blue-400 shadow-lg' : 'text-slate-500'}`}>
              <Briefcase size={14} className="inline mr-2" /> Job Description
            </button>
            <button onClick={() => setActiveTab('resume')} className={`flex-1 py-3 text-xs uppercase font-bold rounded-xl transition-all ${activeTab === 'resume' ? 'bg-slate-800 text-blue-400 shadow-lg' : 'text-slate-500'}`}>
          ...
          <div className="relative flex-1 group">
            ...
            />
            {/* 3. Upload Buttons Restored */}
            <div className="absolute top-4 right-4 flex gap-2">
              <button className="p-2 bg-slate-800 rounded-lg text-slate-400 hover:text-white transition-all shadow-lg border border-white/5">
                <Upload size={14} />
              </button>
              <button onClick={() => activeTab === 'jd' ? setJobDescription('') : setResume('')} className="p-2 bg-slate-800 rounded-lg text-slate-400 hover:text-rose-400 transition-all shadow-lg border border-white/5">
                <Trash2 size={14} />
              </button>
            </div>
          </div>
