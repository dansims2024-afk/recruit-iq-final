  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col font-sans relative">
      ...
      <main className="max-w-7xl mx-auto w-full p-8 grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1">
        ...
        <div className="space-y-6">
          <section className="bg-slate-900 rounded-3xl border border-slate-800 flex flex-col h-[550px] overflow-hidden shadow-2xl">
            ...
            <div className="relative flex-1">
              ...
              {/* RESTORED UPLOAD BUTTONS IN BOX */}
          ...
          {/* SYNERGY MATCH SECTION (Positioned Below Search) */}
          {analysis && (
        ...
        {/* RIGHT COLUMN: LOCKED/UPGRADED TOOLS */}
        <section className="space-y-6 overflow-y-auto pr-2 custom-scrollbar">
          {!analysis ? (
             <div className="h-full border-2 border-dashed border-slate-800 rounded-3xl flex items-center justify-center text-slate-700 uppercase text-[10px] tracking-widest">Awaiting Results...</div>
          ) : (
            <div className="space-y-6">
              {/* Market Intelligence */}
              <div onClick={() => handleFeatureClick('market', 'Comp Intelligence')} className="bg-slate-900 p-8 rounded-3xl border border-slate-800 relative cursor-pointer group">
              ...
              <div onClick={() => handleFeatureClick('email', 'Emailer')} className="bg-slate-900 p-8 rounded-3xl border border-slate-800 relative">
                ...
                <div className="flex items-center justify-between mb-4">
                   <h3 className="text-purple-400 font-bold uppercase text-[10px] tracking-widest flex items-center gap-2"><Send size={14} /> Outreach Email</h3>
                   <button className="bg-purple-600 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase">Generate</button>
                </div>
              </div>

              {/* Interview Guide */}
              <div onClick={() => handleFeatureClick('interview', 'Interview Guide')} className="bg-slate-900 p-8 rounded-3xl border border-slate-800 relative">
                ...
                <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-10 bg-slate-950/50 rounded-xl border border-white/5"></div>)}</div>
              </div>
            </div>
          )}
        </section>
      ...
      <footer className="bg-slate-900 border-t border-white/5 p-8 mt-auto">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-8">
            ...
            <Linkedin size={18} className="text-slate-400 hover:text-purple-400 cursor-pointer" />
            <Github size={18} className="text-slate-400 hover:text-purple-400 cursor-pointer" />
          </div>
