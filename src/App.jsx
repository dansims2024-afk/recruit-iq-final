...
import logo from './logo.png'; // FIXED PATH: Standardized to same directory
...
export default function Dashboard() {
  ...
  // --- ELITE PDF GENERATOR (Fixed Overlap logic) ---
  const downloadPDF = () => {
    if (!analysis) return;
    ...
    // Fixed Column Widths for Strengths & Gaps
  ...
  const handleScreen = async () => {
    ...
    try {
      ...
      const prompt = `Analyze JD: ${jdText} and Resume: ${resumeText}. Extract candidate name, score 0-100, executive summary, 3 strengths, 3 gaps, 5 interview questions, and outreach email. Return ONLY JSON: {"candidate_name": "Name", "score": 0, "summary": "...", "strengths": [], "gaps": [], "questions": [], "outreach_email": "..."}`;
  ...
  return (
    <div className="relative p-6 md:p-10 max-w-7xl mx-auto space-y-8 text-white bg-[#0B1120] min-h-screen pt-20">
      ...
      <div className="grid md:grid-cols-3 gap-6">
          <div onClick={() => setActiveTab('jd')} className={`p-6 rounded-3xl border cursor-pointer transition-all ${jdReady ? 'bg-indigo-900/20 border-emerald-500' : 'bg-slate-800/30 border-slate-700'}`}>
              <div className="flex justify-between items-center mb-3">
                ...
                {jdReady && <span className="text-emerald-400 font-bold text-[9px] uppercase bg-emerald-500/10 px-2 py-1 rounded-md">Validated</span>}
              </div>
          ...
          <div onClick={() => setActiveTab('resume')} className={`p-6 rounded-3xl border cursor-pointer transition-all ${resumeReady ? 'bg-indigo-900/20 border-emerald-500' : 'bg-slate-800/30 border-slate-700'}`}>
              <div className="flex justify-between items-center mb-3">
                ...
                {resumeReady && <span className="text-emerald-400 font-bold text-[9px] uppercase bg-emerald-500/10 px-2 py-1 rounded-md">Validated</span>}
              </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="bg-[#111827] p-8 rounded-[2.5rem] border border-slate-800 flex flex-col h-[850px] shadow-2xl">
            ...
            <div className="flex gap-3 mb-4">
              ...
              <button onClick={() => {setJdText(SAMPLE_JD); setResumeText(SAMPLE_RESUME);}} className="flex-1 bg-slate-800/50 py-3 rounded-xl text-[10px] font-bold uppercase text-slate-400 border border-slate-700 transition-all">Samples</button>
            </div>
            ...
            {analysis ? (
              <div className="space-y-6 animate-in fade-in">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-rose-500/5 border border-rose-500/20 p-6 rounded-3xl text-[11px]"><h4 className="text-rose-400 font-bold uppercase mb-3 text-[9px]">Gaps</h4>{analysis.gaps.map((g, i) => <p key={i}>• {g}</p>)}</div>
                </div>

                <div className="bg-[#111827] border border-slate-800 p-6 rounded-3xl">
                  <h4 className="text-indigo-400 font-bold uppercase text-[9px] mb-4">Strategic Questions</h4>
                ...
                <div className="bg-[#111827] border border-slate-800 p-6 rounded-3xl text-center">
                    ...
                    <button onClick={() => {navigator.clipboard.writeText(analysis.outreach_email); showToast("Copied", "success")}} className="w-full py-3 bg-slate-800 rounded-xl text-[10px] font-black uppercase">Copy to Clipboard</button>
                </div>
              </div>
            ) : (
      ...
      {/* SALES MODAL (RESORED FROM IMAGE_95B5AC.PNG) */}
      {showLimitModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-xl bg-slate-950/80 animate-in fade-in duration-300">
          <div className="relative w-full max-w-lg bg-[#0F172A] border border-slate-700 p-10 rounded-[2.5rem] shadow-2xl text-center">
            <h2 className="text-3xl font-black text-white mb-2 uppercase">Elite Required</h2>
            <p className="text-slate-400 text-sm mb-6 leading-relaxed">You've reached your free scan limit. Start your 3-day free trial to unlock unlimited analysis.</p>
            <a href={finalStripeUrl} className="block w-full py-4 bg-indigo-600 text-white font-bold rounded-xl uppercase tracking-wider shadow-lg hover:bg-indigo-500 transition-colors">Start Free Trial</a>
            <button onClick={() => setShowLimitModal(false)} className="text-[10px] text-slate-600 mt-6 uppercase font-bold tracking-widest hover:text-white transition-colors">Maybe Later</button>
          </div>
        </div>
      )}
