      ...
      {showUpgradeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-[2.5rem] max-w-4xl w-full p-8 relative shadow-2xl">
            <button onClick={() => setShowUpgradeModal(false)} className="absolute top-6 right-6 text-slate-500 hover:text-white">
              <X />
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="p-6 rounded-3xl border-2 bg-slate-950 flex flex-col" style={{ borderColor: brand.primary }}>
                ...
                <button onClick={() => {setSimTier('professional'); setShowUpgradeModal(false);}} className="w-full py-2 rounded-xl text-[10px] font-black uppercase text-white" style={{ backgroundColor: brand.primary }}>Upgrade</button>
              </div>
              <div className="p-6 rounded-3xl bg-slate-950 border border-slate-800">
                ...
                <button onClick={() => {setSimTier('executive'); setShowUpgradeModal(false);}} className="w-full py-2 rounded-xl text-[10px] font-black uppercase text-white" style={{ backgroundColor: brand.accent }}>Go Executive</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto w-full p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section className="bg-slate-900 rounded-[2.5rem] border border-slate-800 flex flex-col h-[700px] overflow-hidden shadow-2xl">
          ...
          <div className="flex justify-between p-3 bg-slate-950/20">
            <button onClick={loadSampleData} className="text-[10px] font-black flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-800 text-slate-400 hover:text-white transition-colors">
              <RefreshCw size={12} /> LOAD SAMPLE IT CASE
            </button>
            <div className="flex gap-2">
              <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept=".pdf" />
              <button onClick={() => fileInputRef.current.click()} className="text-[10px] font-black flex items-center gap-2 px-4 py-1.5 rounded-full border border-slate-700 text-slate-500 hover:text-white">
          ...
          {!analysis ? (
            ...
            <div className="space-y-6 animate-in fade-in duration-700">
              <div className="bg-slate-900 rounded-[2.5rem] border border-slate-800 shadow-xl overflow-hidden">
                ...
                <div className="p-10">
                  <h3 className="text-[10px] font-black uppercase tracking-widest mb-4 flex items-center gap-2" style={{ color: brand.neutral }}>
                    <FileText size={14} /> Executive Fit Summary
                  </h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-emerald-500/5 p-6 rounded-3xl border border-emerald-500/20">
                    ...
                    {analysis.strengths.map((s, i) => (
                      <div key={i} className="text-xs text-slate-400 mb-1 leading-relaxed">✓ {s}</div>
                    ))}
                  </div>
                  <div className="bg-rose-500/5 p-6 rounded-3xl border border-rose-500/20">
                    ...
                    {analysis.gaps.map((g, i) => (
                      <div key={i} className="text-xs text-slate-400 mb-1 leading-relaxed">! {g}</div>
                    ))}
                  </div>
                </div>
              )}
              {simTier !== 'standard' && (
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2" style={{ color: brand.primary }}>
                      <Mail size={14} /> Personalized Outreach
                    </h3>
                    <button onClick={() => {navigator.clipboard.writeText(outreachDraft); alert('Email Copied!')}} className="text-slate-500 hover:text-white">
                      <Copy size={14} />
                    </button>
                  </div>
